            ...
            for i in range(50):
                ...
                for key in results.get('bbox_fields', []):
                    ...
                    if key in ['gt_bboxes']:
                        ...
                        if 'gt_masks' in results and self.copyPaste == False:
                            raise NotImplementedError(
...
class CopyPaste:
    ...
    def _copy_paste(self, dst_results, src_results):
        ...
        dst_results['img'] = img.astype(np.float32)
...
class MVACopyPaste:
    ...
    def __init__(
        ...
        self.bbox_path = "data/mva2023_sod4bird_train/cropped_bbox/"
...
@PIPELINES.register_module()
class MVACopyPasteSameImage:
    """
    copy paste objects in the same image 0~2 times
    """
    ...
    def __call__(self, results):
        ...
        for box in results["gt_bboxes"]:
            ...
            bbox_imgs.append(new_img[box[1]:box[3], box[0]:box[2]])

        for i in range(len(new_bboxes)):
            n = random.randint(2)
            img_size = bbox_imgs[i].shape[:2]
            for _ in range(n):
                scale_factor = random.uniform(0.5, 2)
                box_h, box_w = int(img_size[0] * scale_factor), int(img_size[1] * scale_factor)
                if box_w > w or box_h > h or box_w <= 0 or box_h <= 0: continue
                if (random.randint(2)):
                    img = cv2.flip(bbox_imgs[i], 1)
                else:
                    img = bbox_imgs[i]
                img = cv2.resize(img, dsize=(box_w, box_h))
                x1, y1 = random.randint(w-box_w), random.randint(h-box_h)
                x2, y2 = x1 + box_w, y1 + box_h
                new_img[y1:y2, x1:x2] = img
                append_bboxes.append([x1, y1, x2, y2])
        ...
        if len(append_bboxes):
            new_bboxes = np.concatenate([new_bboxes, append_bboxes], dtype=np.float32)
            new_labels = np.concatenate([new_labels, [0]*len(append_bboxes)], dtype=int)
...
class MVAPasteBirds:
    ...
    def __call__(self, results):
        ...
        for _ in range(n):
            ...
            box_w = round(random.uniform(5, 80))
